...
# FUNCTIONS ADAPTED FROM ADVERTORCH
...
class MaskedPGDAttack(PGDAttack):
    ...
    def __init__(
            ...
            ord=np.inf, l1_sparsity=None, targeted=False, mask_steps=100, device="cuda:0", experiment_name="experiment_unnamed"):
        """
...
def main():
    ...
    adversary = MaskedPGDAttack(
        model, loss_fn=nn.CrossEntropyLoss(reduction="sum"), eps=.03,
        nb_iter=2, eps_iter=0.01, rand_init=True, clip_min=0.0, clip_max=1.0,
        targeted=False)

    inputs = torch.randn(4,3,180,180).cuda()
    y = torch.Tensor([1,3,0,1]).long().cuda()

    output = adversary.perturn(inputs, y)
    print(output.shape)
